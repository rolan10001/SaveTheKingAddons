...
public class Explorer extends RolesImpl implements Power {
	
	private final Category category = Category.SERVANT;
	private boolean power = true;
	
	public Explorer(STKGameManager game, @NotNull UUID uuid) {
		super(game, uuid);
	}

	@Override
	public String getDescription() {
		return STKPlugin.getInstance().getSTKConfig().getString("stk.role.explorer.description");
	}

	@Override
	public String getDisplay() {
		return STKPlugin.getInstance().getSTKConfig().getString("stk.role.explorer.display");
	}
	...
	public void stolen(@NotNull UUID uuid) {
		...
		player.getInventory().addItem(setMetaInItem(new ItemStack(Material.COOKED_BEEF, 24), "§fSteak du Servant", null));
